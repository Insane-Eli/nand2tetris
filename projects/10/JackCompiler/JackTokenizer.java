...
public class JackTokenizer {
	...
	*/
	...
	/** hasMoreTokens will advance the cursor to a valid character. It will skip over comments
		and multi-line comments until it finds a valid token character or reaches the end
		of the file.  **/
	...
	/** JackTokenizer initially has no current token. hasMoreTokens() will always get called
		prior to advance() and advance() will only get called if hasMoreTokens() returns
		true. We can assume that if advance is getting called, the cursor must be positioned
		at the start of a valid token. advance() looks at the current character to determine
		the type of token and will build a string while incrementing the cursor location
	...
	/** parseInt() will form an integer value by collecting characters one at a time 
		(as long as the character is another digit). The resultant value is stored
		in a variable to be used in the future as an integerConstant lexical
		element. **/
	...
	/** parseIdentifier() will collect a sequence of letter, digits and underscores ('_')
		not starting with a digit. The resultant value is as a String. **/
	...
	/** parseString() will collect a sequence of Unicode characters not including double
		quote or newline and store the value in a state variable for the current
		stringConstant. **/
